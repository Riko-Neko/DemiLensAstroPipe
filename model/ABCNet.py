...
class ABCNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.deep_supervision:
            d_s1 = self.conv1(d2)
            d_s2 = _upsample_like(self.conv2(d3), d_s1)
            d_s3 = _upsample_like(self.conv3(d4), d_s1)
            d_s4 = _upsample_like(self.conv4(d5), d_s1)
            d_s5 = _upsample_like(self.conv5(e5), d_s1)
