...
class MultiHeadChannelAttention(nn.Module):
    def __init__(self, in_channels, H, W, num_heads = 4, reduction = 16):
        super().__init__()
        self.H = H
        self.W = W
        self.num_heads = num_heads
        ...
        self.mlp = nn.Sequential(
            nn.Conv2d(in_channels, in_channels // reduction, 1),  # Changed reduction factor
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        x_head = x_2d.view(B * self.num_heads, self.head_dim, self.H, self.W)

        avg_out = self.avg_pool(x_head)  # [B*num_heads, head_dim, 1, 1]
        max_out = self.max_pool(x_head)  # [B*num_heads, head_dim, 1, 1]

        # Combine avg and max pooling
        pooled = avg_out + max_out  # [B*num_heads, head_dim, 1, 1]

        # Reshape to combine batch and heads with channels for MLP
        pooled = pooled.view(B, self.num_heads * self.head_dim, 1, 1)  # [B, C, 1, 1]

        attn = self.mlp(pooled)  # [B, C, 1, 1]

        # Reshape attention weights back to multi-head format
        attn = attn.view(B, self.num_heads, self.head_dim, 1, 1)  # [B, num_heads, head_dim, 1, 1]

        x_head = x_head.view(B, self.num_heads, self.head_dim, self.H, self.W)
        x_out = x_head * attn  # [B, num_heads, head_dim, H, W]

        x_out = x_out.view(B, C, self.H, self.W)

        return x_out.permute(0, 2, 3, 1).contiguous().view(B, L, C)
...
class DemiLensesSwin(nn.Module):
    ...
    def __init__(self, img_size = 96, patch_size = 4, in_chans = 3, num_classes = 1, embed_dim = 64,
                 depths = [2, 2, 6, 2], num_heads = [4, 8, 16, 32], window_size = 6, mlp_ratio = 4., qkv_bias = True,
        ...
        self.embed_dim = embed_dim
        self.ape = ape
        self.norm_layer = DyT if norm_layer == 'DyT' else norm_layer
