...
class FcaAfnoBlock(nn.Module):
    ...
    def __init__(self, channels, height, width, reduction=16, groups=2, init_std=0.005, vis=[False, None]):
        ...
        self.height = height
        self.width = width
        # Frequency Channel Attention submodule
        self.fca = FrequencyChannelAttention(channels, height, width, reduction, groups)
        # FFT frequency filter parameters: learnable frequency domain weights (real and imaginary numbers), size (1, C, H, W//2+1)
        # 复数频率滤波器
        self.freq_filter_real = nn.Parameter(torch.ones(1, channels, height, width // 2 + 1))
        self.freq_filter_imag = nn.Parameter(torch.zeros(1, channels, height, width // 2 + 1))

        self.scale = nn.Parameter(torch.ones(1, 1, 1, 1))

        nn.init.normal_(self.freq_filter_real, mean=1.0, std=init_std)
        nn.init.normal_(self.freq_filter_imag, mean=0.0, std=init_std)
    ...
    def forward(self, x):
        ...
        # Perform 2D real FFT on x_attn
        Xf = torch.fft.rfft2(x_attn, dim=(-2, -1))  # (B, C, H, W//2+1), dtype=complex
        # Extend the weights and apply to the spectrum
        # 复数乘法
        weight = torch.complex(self.freq_filter_real, self.freq_filter_imag)
        magnitude = torch.abs(weight)
        normalized_weight = weight / (magnitude + 1e-10)  # 单位化
        Xf = normalized_weight * self.scale * Xf
...
if __name__ == '__main__':
    ...
    x = torch.stack([generate_wave_image(96, 96) for _ in range(8)], dim=0)  # shape (8, 3, 96, 96)
    model = DemiLensNetDev(ori_h=96, dim=32, extra_fc=True, ablated=[True, True], visualize=False)
